...
namespace Tests.ContollerTests
{
    ...
    {
        private readonly IFinnhubSotckService _finnhubStockService;
        private readonly Mock<IFinnhubSotckService> _finnhubStockServiceMock;
        private readonly Fixture _fixture;

        public StocksControllerTest()
        {
            _fixture = new Fixture();
            _finnhubStockServiceMock = new Mock<IFinnhubSotckService>();
            _finnhubStockService = _finnhubStockServiceMock.Object;
        }
        ...
        public async Task Explore_StockIsNull_ShouldReturnExploreViewWithStocksList()
        {
            ...
            StocksController stocksController = new StocksController(tradingOptions, _finnhubStockService);
            ...
            _finnhubStockServiceMock.Setup(temp => temp.GetStocks()).ReturnsAsync(stocksDict);

            var exptectedStocks = stocksDict!.Select(temp => new Stock() { StockName = Convert.ToString(temp["description"]), StockSymbol = Convert.ToString(temp["symbol"]) })
                .ToList();
            ...
            viewResult.ViewData.Model.Should().BeEquivalentTo(exptectedStocks);
